...
const SERVER_DB_PATH = process.env.SERVER_DB_PATH || 'db-users.json';
...
class Controller {
    ...
    get(id) {
        ...
        return new Promise((resolve, reject) => {
            resolve( this.db.findAll( id ? { userId: id } : {} ) || reject( {} ).value() );
        });
    }
    ...
    update(id, data) {
        ...
        return new Promise((resolve, reject) => {
            resolve( this.db.chain().find({ userId: id }).assign( data ).value() );
        });
    }
    ...
    delete(id) {
        ...
        return new Promise((resolve, reject) => {
            resolve( this.db.remove({ userId : id }).value() );
        });
    }
}
